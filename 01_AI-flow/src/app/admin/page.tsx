...
export default function AdminDashboard() {
  return (
    <div className="font-body-md text-gray-900 min-h-screen bg-[#f8f9fa] flex flex-col">
        ...
        <aside className="w-72 bg-white border-r border-gray-200 flex flex-col shrink-0">
          <div className="p-8">
            ...
            <nav className="flex flex-col gap-2">
              <div className="mb-4">
                <a href="/admin" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50 text-emerald-tech font-bold text-sm border border-emerald-tech/20 transition-all cursor-pointer">
              ...
              <div className="flex flex-col gap-1">
                {[
                  ...
                  <div key={i} className="flex flex-col">
                    <a href={item.href} className="flex items-center justify-between px-4 py-3 rounded-xl text-gray-500 hover:bg-gray-50 hover:text-gray-900 transition-all text-sm cursor-pointer group">
                    ...
                    {item.sub && i < 2 && (
                       <div className="flex flex-col ml-11 mt-1 mb-2 gap-2">
                         {item.sub.map(s => (
                           <a key={s} href="#" className="text-xs text-gray-400 hover:text-emerald-tech transition-colors">{s}</a>
                         ))}
                       </div>
                    )}
                  </div>
          ...
          <div className="mt-auto p-8">
             <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4 flex items-center justify-between group hover:border-emerald-tech/30 transition-all cursor-pointer">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[11px] font-bold text-gray-900">Zippy Root Admin</p>
                    <p className="text-[9px] text-gray-400 uppercase tracking-tighter">Master Access</p>
                  </div>
                </div>
          ...
          <div className="max-w-7xl mx-auto flex flex-col gap-10">
              ...
              <div className="flex items-center gap-6">
                <div className="relative">
                  ...
                  <input 
                    type="text" 
                    placeholder="시스템 검색..." 
                    className="bg-white border border-gray-200 rounded-lg py-2 pl-10 pr-4 text-xs text-gray-900 w-64 focus:ring-1 focus:ring-emerald-tech focus:border-emerald-tech" 
                  />
                </div>
                <div className="relative">
            ...
            <section>
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-[10px] text-gray-400 uppercase tracking-widest">마지막 업데이트: 1분 전</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  ...
                  <div key={i} className="bg-white border border-gray-200 rounded-2xl p-6 flex flex-col gap-4">
                    <p className="text-[10px] text-gray-500 uppercase tracking-widest font-technical-sm">{stat.label}</p>
                    <div className="flex items-end justify-between">
                      ...
                      {stat.status && (
                        <span className={`text-[10px] font-bold ${stat.color} px-2 py-0.5 bg-gray-50 rounded uppercase`}>
                          {stat.status}
                        </span>
                      )}
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              ...
              <div className="lg:col-span-5 flex flex-col gap-10">
                <section className="bg-white border border-gray-200 rounded-3xl p-8">
                  <div className="flex items-center justify-between mb-8">
                    <h3 className="font-bold text-gray-900 text-sm">금일 요약 업데이트 현황</h3>
                    <RefreshCcw className="w-4 h-4 text-gray-400 cursor-pointer hover:text-emerald-tech" />
                  </div>
                  <div className="flex flex-col gap-4">
                    {[
                      ...
                      <div key={i} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-200 group hover:border-gray-300 transition-all cursor-pointer">
                         <div className="flex items-center gap-4">
                           <span className="text-xl font-bold text-gray-400 group-hover:text-emerald-tech transition-colors">{item.id}</span>
                           <div>
                             <p className="text-sm font-bold text-gray-900">{item.title}</p>
                             <p className="text-[10px] text-gray-400 font-technical-sm">{item.sub}</p>
                           </div>
                         </div>
                         <span className={`text-[9px] font-bold px-2 py-1 rounded uppercase ${
                           item.status === '완료' ? 'text-emerald-tech bg-emerald-tech/10' :
                           item.status === '진행중' ? 'text-blue-400 bg-blue-400/10' : 'text-gray-400 bg-gray-100'
                         }`}>{item.status}</span>
                      </div>
                    ))}
                  </div>
                ...
                <section className="bg-white border border-gray-200 rounded-3xl p-8">
                  <h3 className="font-bold text-gray-900 text-sm mb-8">북마크 급상승 순위 (TOP 3)</h3>
                  <div className="flex flex-col gap-4">
                    {[
              ...
              <div className="lg:col-span-7">
                <section className="bg-white border border-gray-200 rounded-3xl p-8 h-full flex flex-col">
                  <div className="flex items-center justify-between mb-8">
                    <h3 className="font-bold text-gray-900 text-sm">최근 학습 완료 유저 및 학점(CREDIT) 발생</h3>
                    <div className="flex items-center gap-2 bg-gray-50 border border-gray-300 rounded-lg px-3 py-1.5 cursor-pointer">
                      <span className="text-[10px] text-gray-600 font-bold uppercase">전체 사이트</span>
                    ...
                    <table className="w-full text-left">
                      <thead>
                        <tr className="border-b border-gray-200">
                          <th className="pb-4 text-[10px] text-gray-400 uppercase tracking-widest font-technical-sm">유저 ID</th>
                          <th className="pb-4 text-[10px] text-gray-400 uppercase tracking-widest font-technical-sm">학습 코스</th>
                          <th className="pb-4 text-[10px] text-gray-400 uppercase tracking-widest font-technical-sm">발생 학점</th>
                          <th className="pb-4 text-[10px] text-gray-400 uppercase tracking-widest font-technical-sm text-right">처리 시각</th>
                          <th className="pb-4 text-[10px] text-gray-400 uppercase tracking-widest font-technical-sm text-right">상태</th>
                        </tr>
                      </thead>
                      <tbody>
                        {[
                          ...
                          <tr key={i} className="border-b border-gray-200 last:border-0 group hover:bg-gray-50 transition-colors cursor-pointer">
                            <td className="py-4 text-xs text-gray-600 font-technical-sm">{row.id}</td>
                            <td className="py-4 text-xs font-bold text-gray-900">{row.course}</td>
                            <td className="py-4 text-xs font-bold text-orange-400">{row.credits}</td>
                            <td className="py-4 text-[10px] text-gray-400 font-technical-sm text-right">{row.time}</td>
                  ...
                  <div className="mt-8 pt-6 border-t border-gray-200 text-center">
                    <button className="text-[10px] font-bold text-gray-500 hover:text-emerald-tech transition-colors uppercase tracking-widest cursor-pointer">활동 로그 전체 보기</button>
                  </div>
                </section>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
               <section className="lg:col-span-7 bg-white border border-gray-200 rounded-3xl p-8">
               ...
               <section className="lg:col-span-5 bg-white border border-gray-200 rounded-3xl p-8">
                  <h3 className="font-bold text-gray-900 text-sm mb-8">빠른 시스템 도구</h3>
                  <div className="grid grid-cols-2 gap-4">
                     <button className="flex items-center justify-center gap-3 p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-emerald-tech/30 transition-all text-[11px] font-bold text-gray-900 cursor-pointer group">
      ...
      {/* Footer */}
      <footer className="w-full bg-[#f8f9fa] border-t border-gray-200 px-12 py-4 flex items-center justify-between">
         <p className="text-[10px] text-gray-400 uppercase tracking-widest">© 2024 A-ZIP UNIFIED SYSTEMS. 모든 권리 보유.</p>
         <div className="flex gap-8 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
            <a href="#" className="hover:text-gray-900 transition-colors">운영 정책</a>
            <a href="#" className="hover:text-gray-900 transition-colors">시스템 모니터링</a>
         </div>
      </footer>
    </div>
  );
